...
const ViewPOModal = ({ open, onClose, po }) => {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case "Approved":
        return { bg: "#e0f2fe", color: "#14b8a6", border: "#bae6fd" };
      case "Pending":
        return { bg: "#fffbeb", color: "#f59e0b", border: "#fde68a" };
      case "Received":
        return { bg: "#f0fdf4", color: "#10b981", border: "#bbf7d0" };
      case "Draft":
  ...
  const poTotal =
    po.lineItems?.reduce((sum, item) => sum + item.total, 0) ?? po.total;
        ...
        {po.invoice?.status !== "none" && (
          <>
            ...
            >
              ...
              <Box>
                <Typography
                  sx={{ fontSize: 13, fontWeight: 700, color: "#15803d" }}
                >
                  Invoice Attached
                </Typography>
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Typography
            sx={{ fontSize: 14, fontWeight: 700, color: "#111827", mb: "4px" }}
          >
            {invoice.number}
          </Typography>
          <Typography sx={{ fontSize: 11, color: "#9ca3af", mb: "12px" }}>
        ...
        >
          <Typography sx={{ fontSize: 11, color: "#9ca3af" }}>
            Uploaded by:{" "}
            <strong style={{ color: "#374151" }}>{invoice.verifiedBy}</strong>
          </Typography>
          <Typography sx={{ fontSize: 11, color: "#9ca3af" }}>
            {invoice.uploadedAt}
          </Typography>
...
const PurchaseOrders = () => {
  ...
  const [purchaseOrders, setPurchaseOrders] = useState([
    {
      ...
      lineItems: [
        ...
        {
          description: "Epinephrine 1mg/mL 10mL",
          quantity: 20,
          unitCost: 18.5,
          total: 370.0,
        },
    ...
    },
    {
      ...
      lineItems: [
        ...
        {
          description: "Face Masks (Box of 50)",
          quantity: 15,
          unitCost: 16.0,
          total: 240.0,
        },
  ...
  const stats = {
    ...
    pendingApproval: purchaseOrders.filter((po) => po.status === "Pending")
      .length,
    approved: purchaseOrders.filter((po) => po.status === "Approved").length,
    received: purchaseOrders.filter((po) => po.status === "Received").length,
    invoicesAttached: purchaseOrders.filter(
      (po) => po.invoice.status !== "none",
    ).length,
  };
  ...
  const handleSavePO = (poData) => {
    const supplierLabel =
      suppliers.find((s) => s.value === poData.supplier)?.label || "";
    const locationLabel =
      locations.find((l) => l.value === poData.deliverTo)?.label || "";
    const newPO = {
      id: poData.poNumber,
      ...
      delivery: new Date(poData.requiredDelivery).toLocaleDateString("en-US", {
        month: "short",
        day: "numeric",
        year: "numeric",
      }),
    ...
    alert(
      `Purchase Order ${poData.poNumber} saved as ${poData.status === "Draft" ? "Draft" : "Pending"} successfully!`,
    );
  };
  ...
  const handleSaveInvoice = (invoiceData) => {
    ...
    alert(
      `Invoice ${invoiceData.invoiceNumber} uploaded successfully for ${invoiceData.poNumber}!`,
    );
  };
      ...
      >
        <Box>
          ...
          <Typography sx={{ fontSize: 12, color: "#9ca3af", mt: "4px" }}>
            {stats.totalPOs} POs · {stats.pendingApproval} pending approval ·{" "}
            {stats.invoicesAttached} invoices attached
          </Typography>
        </Box>
        <Button
          variant="contained"
