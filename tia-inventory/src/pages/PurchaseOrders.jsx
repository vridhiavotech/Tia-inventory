...
const ViewPOModal = ({ open, onClose, po }) => {
  ...
  const priorityStyle = getPriorityColor(po.priority);
    ...
    >
      ...
      <DialogContent sx={{
        ...
        <Box sx={{ display: 'flex', gap: 1, mb: '20px' }}>
          ...
          <Chip label={po.priority} size="small" sx={{
            bgcolor: priorityStyle.bg, color: priorityStyle.color,
        ...
        <Box sx={{
          ...
          {[
            ...
            <Box key={label}>
              ...
              <Typography sx={{ fontSize: 13, fontWeight: 600, color: valueColor || '#111827' }}>
                {value}
              </Typography>
            </Box>
          ))}
        ...
        <Box sx={{ mb: '20px' }}>
          ...
          <Box sx={{
            display: 'grid',
            gridTemplateColumns: 'minmax(0,3fr) 52px 72px 72px',
            gap: '6px', mb: '8px', px: '10px',
          }}>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            {po.lineItems?.map((item, idx) => (
              ...
              >
                ...
                <Typography sx={{ fontSize: 13, color: '#6b7280', textAlign: 'right' }}>
                  {item.quantity}
                </Typography>
                <Typography sx={{ fontSize: 13, color: '#6b7280', textAlign: 'right' }}>
  ...
  return (
    ...
    >
      ...
      <Box sx={{
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <Box sx={{
            width: 38, height: 38, borderRadius: '10px', background: '#f0fdf4',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontSize: 20,
          }}>
            🧾
      ...
      <DialogContent sx={{
        ...
        <Box sx={{
          ...
          border: '1px solid #bbf7d0', bgcolor: '#f0fdf4', mb: '16px',
        }}>
        ...
        <Box sx={{
          display: 'flex', alignItems: 'center', gap: '12px',
          p: '12px 16px', borderRadius: '10px',
          border: '1px solid #e5e7eb', bgcolor: '#f9fafb', mb: '16px',
        }}>
          <Typography sx={{ fontSize: 20 }}>📕</Typography>
          <Box sx={{ flex: 1 }}>
        ...
        <Box sx={{
          ...
          border: '1px solid #e5e7eb', bgcolor: '#fff', mb: '16px',
        }}>
          <Box>
      ...
      <Box sx={{
        ...
        <Button
          disableRipple
          sx={{ ...btnBase, color: '#ef4444', border: '1px solid #fca5a5', bgcolor: '#fff', '&:hover': { bgcolor: '#fef2f2' } }}
        >
          🚩 Mark Disputed
        </Button>
        <Button
...
const PurchaseOrders = () => {
  ...
  const [purchaseOrders, setPurchaseOrders] = useState([
    ...
    },
    {
      ...
      invoice: {
        ...
        dueDate: 'May 4, 2026', amount: 696.00, paymentTerms: 'Net-45',
  ...
  const statCards = [
    { title: 'Total Purchase Orders', value: stats.totalPOs,        subtitle: `${stats.totalPOs} orders · $${stats.totalValue.toLocaleString()} total value`, color: '#3b82f6' },
    { title: 'Pending Approval',      value: stats.pendingApproval, subtitle: 'Awaiting approval',   color: '#f59e0b' },
    { title: 'Approved / In Transit', value: stats.approved,        subtitle: 'Ready to receive GRN', color: '#10b981' },
    { title: 'Received',              value: stats.received,        subtitle: 'Completed orders',    color: '#8b5cf6' },
  ];
  ...
  const handleApprove = (id) => setPurchaseOrders(purchaseOrders.map(po => po.id === id ? { ...po, status: 'Approved' } : po));
  ...
  const handleSavePO = (poData) => {
    ...
    const newPO = {
      id: poData.poNumber, quotRef: poData.quotationRef || '—',
      supplier: supplierLabel, location: locationLabel,
      lines: poData.lineItems.length, total: poData.totalAmount,
      ...
      priority: poData.priority, status: poData.status === 'Draft' ? 'Draft' : 'Pending',
      invoice: { status: 'none', number: null },
    ...
    setPurchaseOrders(purchaseOrders.map(po =>
      po.id === invoiceData.poNumber
        ? {
            ...po,
            invoice: {
              status: 'verified', number: invoiceData.invoiceNumber,
              date: invoiceData.invoiceDate, dueDate: invoiceData.paymentDueDate,
              amount: invoiceData.invoiceAmount, paymentTerms: invoiceData.paymentTerms,
              fileName: invoiceData.file?.name || 'invoice.pdf', fileSize: '184 KB',
              verifiedBy: 'Current User', notes: invoiceData.notes,
    ...
    <Box sx={{ background: '#f8f9fb', minHeight: '100vh', p: '28px 32px', boxSizing: 'border-box' }}>
      ...
      <Box sx={{ display: 'flex', gap: '12px', mb: '20px' }}>
        {statCards.map((card, index) => (
          <Box key={index} sx={{
            ...
            borderLeft: `3px solid ${card.color}`, borderRadius: '10px',
            ...
            <Typography sx={{ fontSize: 11, fontWeight: 600, color: card.color, mt: 0.3 }}>
              {card.subtitle}
            </Typography>
          </Box>
        ))}
      ...
      <Box sx={{ display: 'flex', gap: 1.5, mb: '20px', flexWrap: 'wrap' }}>
        <Box sx={{
          ...
          p: '8px 13px', background: '#fff',
        }}>
          <Typography sx={{ fontSize: 16 }}>🧾</Typography>
          <Typography sx={{ color: '#9ca3af', fontSize: 12 }}>Invoices attached:</Typography>
          <Typography sx={{ color: '#14b8a6', fontWeight: 600, fontSize: 12 }}>
          ...
          <Table sx={{ minWidth: 1100 }}>
            <TableHead>
              <TableRow sx={{ background: '#f8f9fb' }}>
                {['PO NUMBER','QUOT. REF','SUPPLIER','LOCATION','LINES','TOTAL','CREATED BY','DATE','DELIVERY','PRIORITY','STATUS','INVOICE','ACTIONS'].map(col => (
              ...
              {purchaseOrders.map((po, idx) => {
                ...
                return (
                  <TableRow key={po.id} sx={{
                    ...
                    '& td': { borderBottom: idx < purchaseOrders.length - 1 ? '1px solid #f3f4f6' : 'none', py: '14px', px: '16px' },
                  }}>
                    <TableCell><Typography sx={{ color: '#14b8a6', fontWeight: 700, fontSize: 12 }}>{po.id}</Typography></TableCell>
                    <TableCell>
                      <Typography sx={{ fontSize: 12, fontWeight: 600, color: '#111827' }}>{po.quotRef}</Typography>
                      <Typography sx={{ fontSize: 11, color: '#9ca3af' }}>Quot. Ref</Typography>
                    </TableCell>
                    <TableCell><Typography sx={{ fontSize: 13, fontWeight: 500, color: '#111827' }}>{po.supplier}</Typography></TableCell>
                    <TableCell>
                    ...
                    <TableCell><Typography sx={{ fontSize: 13, color: '#6b7280' }}>{po.lines}</Typography></TableCell>
                    <TableCell><Typography sx={{ fontSize: 13, fontWeight: 600, color: '#111827' }}>${po.total.toLocaleString()}</Typography></TableCell>
                    <TableCell><Typography sx={{ fontSize: 12, color: '#374151' }}>{po.createdBy}</Typography></TableCell>
                    <TableCell><Typography sx={{ fontSize: 12, color: '#6b7280', whiteSpace: 'nowrap' }}>{po.date}</Typography></TableCell>
                    <TableCell><Typography sx={{ fontSize: 12, color: '#6b7280', whiteSpace: 'nowrap' }}>{po.delivery}</Typography></TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <Box sx={{ display: 'flex', gap: '4px' }}>
                        ...
                        <Tooltip title={po.invoice.status === 'none' ? 'Upload Invoice' : 'View Invoice'}>
                          <IconButton size="small" onClick={() => po.invoice.status === 'none' ? handleUploadInvoice(po) : handleViewInvoice(po)}
                            sx={{ color: '#6b7280', border: '1px solid #e5e7eb', borderRadius: '6px', width: 28, height: 28, '&:hover': { background: '#f0fdfa', color: '#0d9488', borderColor: '#99f6e4' } }}>
