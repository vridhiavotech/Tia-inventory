...
const CustomPieTooltip = ({ active, payload }) => {
  ...
  return (
    <Box sx={{ bgcolor:"#fff", border:"1px solid #e5e7eb", borderRadius:"8px", px:1.5, py:0.8, boxShadow:"0 4px 12px rgba(0,0,0,0.08)", fontSize:13, fontWeight:700, color:data.payload.color }}>
      {pct}%
    </Box>
  );
};
...
const statCards = [
  { label:"Inventory Turnover", value:"8.4×",  sub:"vs 7.2× prior year",       iconBg:"#10b981", icon:<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg> },
  { label:"Fill Rate",          value:"97.8%", sub:"↑ 0.4% vs last month",      iconBg:"#2563eb", icon:<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg> },
  { label:"Waste / Shrinkage",  value:"$4,820",sub:"↑ $620 due to expiries",    iconBg:"#ef4444", icon:<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 17 9 11 13 15 21 7"/><polyline points="15 7 21 7 21 13"/></svg> },
  { label:"Stockout Events",    value:"3",     sub:"↑ 1 vs last month",          iconBg:"#f59e0b", icon:<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg> },
];

function SectionCard({ title, subtitle, children }) {
  return (
    <Paper elevation={0} sx={{ border:"1px solid #f0f0f0", borderRadius:"14px", boxShadow:"0 1px 4px rgba(0,0,0,0.04)", overflow:"hidden", bgcolor:"#fff" }}>
      <Box sx={{ display:"flex", alignItems:"center", justifyContent:"space-between", px:"28px", py:"14px", borderBottom:"1px solid #f3f4f6", background:"#fafafa" }}>
        <Box>
  ...
  return (
    <Box>
      ...
      <Box sx={{ display:"flex", alignItems:"center", justifyContent:"space-between", mb:"20px" }}>
        <Box>
          <Typography sx={{ fontSize:20, fontWeight:700, color:"#111827" }}>Reports &amp; Analytics</Typography>
          <Typography sx={{ fontSize:12, color:"#9ca3af", mt:"4px" }}>FY 2026 inventory performance</Typography>
        </Box>
        <Stack direction="row" spacing={1.5}>
          {/* ── All export buttons — Replacement outlined style ── */}
          <Button startIcon={<FileDownload sx={{ fontSize:16 }} />} variant="outlined" onClick={exportInventoryCSV} sx={btnOutlined}>
            Inventory CSV
          </Button>
          <Button startIcon={<FileDownload sx={{ fontSize:16 }} />} variant="outlined" onClick={exportPOCSV} sx={btnOutlined}>
            PO CSV
          </Button>
          {/* ── Full Report — Replacement primary style ── */}
          <Button startIcon={<FileDownload sx={{ fontSize:16 }} />} variant="contained" onClick={exportFullReport} sx={btnPrimary}>
            Full Report
          </Button>
        </Stack>
      </Box>
          ...
          <Box key={s.label} sx={{ flex:1, bgcolor:"#fff", border:"1px solid #e5e7eb", borderRadius:"10px", px:2, py:1.5, minWidth:0, display:"flex", alignItems:"center", gap:1.5 }}>
            ...
            <Box sx={{ minWidth:0 }}>
              ...
              <Box sx={{ display:"flex", alignItems:"baseline", gap:0.5 }}>
                <Typography sx={{ fontSize:typeof s.value === "string" && s.value.length > 8 ? 15 : 22, fontWeight:600, color:"#111827", lineHeight:1.2, whiteSpace:"nowrap", flexShrink:0 }}>{s.value}</Typography>
                <Typography sx={{ fontSize:11, fontWeight:500, color:"#6b7280", whiteSpace:"normal", wordBreak:"break-word" }}>{s.sub}</Typography>
              </Box>
            </Box>
          </Box>
        ))}
