...
// ── Edit User Dialog ───────────────────────────────────────────────────────
const ROLES = ["Location Manager","Manager","Pharmacist","Nurse","Storekeeper","Doctor","Admin"];
...
const SECTION_LABEL_SX = { fontSize:11, fontWeight:700, color:"#7C3AED", letterSpacing:0.8, textTransform:"uppercase", mb:1.5 };
    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth
      ...
      <Box sx={{ px:"24px", pt:"20px", pb:"16px", display:"flex", alignItems:"flex-start",
        ...
        <Box sx={{ display:"flex", alignItems:"center", gap:"12px" }}>
          <Box sx={{ width:38, height:38, borderRadius:"10px", bgcolor:"#EDE9FE",
          ...
          <Box>
            ...
            <Typography sx={{ fontSize:12, color:"#9ca3af", mt:"1px" }}>Update credentials, role & location</Typography>
          </Box>
        </Box>
      ...
      <DialogContent sx={{ px:"24px", py:"20px", overflowY:"auto", maxHeight:"70vh",
        ...
        <Grid container spacing={1.5} sx={{ mb:2 }}>
          ...
          <Grid item xs={12}>
            ...
            <TextField fullWidth size="small" value={localForm.department} onChange={set("department")} sx={FIELD_SX} />
          </Grid>
        </Grid>
        ...
        <Grid container spacing={1.5} sx={{ mb:2 }}>
          <Grid item xs={6}>
            ...
            <Select fullWidth size="small" value={localForm.role} onChange={set("role")} sx={SELECT_SX}>
          ...
          <Grid item xs={6}>
            ...
            <Select fullWidth size="small" value={localForm.location} onChange={set("location")} displayEmpty sx={SELECT_SX}>
              ...
              {locations.map(l => <MenuItem key={l.id} value={l.name} sx={{ fontSize:13 }}>{l.name}</MenuItem>)}
            </Select>
          </Grid>
        </Grid>

        <Typography sx={FIELD_LABEL_SX}>Notes</Typography>
        <TextField fullWidth multiline rows={2} size="small" placeholder="Any notes..."
          value={localForm.notes} onChange={set("notes")}
      ...
      <Box sx={{ px:"24px", py:"16px", borderTop:"1px solid #f3f4f6", display:"flex",
        ...
        <Button onClick={onClose}
          sx={{ fontSize:13, fontWeight:600, color:"#374151", textTransform:"none", borderRadius:"8px",
            px:"20px", py:"9px", border:"1px solid #e5e7eb", background:"#fff", "&:hover":{ background:"#f9fafb" } }}>
          Cancel
        </Button>
...
const SEED_USERS = [
  {
    id: "SA", initials: "SA", avatarBg: "#1976D2",
    name: "Sarah Anderson", username: "@sarah.a", email: "s.anderson@hospital.org",
    role: "Manager",     roleBg: "#EFF6FF", roleColor: "#1D4ED8",
    locationCode: "CS-01", locationName: "Central Store",
    locationBg: "#EFF6FF", locationColor: "#1D4ED8", locationBorder: "#BFDBFE",
    department: "Supply Chain", status: "Active",
  },
  {
    id: "PC", initials: "PC", avatarBg: "#059669",
    name: "P. Chen", username: "@p.chen", email: "p.chen@hospital.org",
    role: "Pharmacist",  roleBg: "#F0FDF4", roleColor: "#15803D",
    locationCode: "PH-01", locationName: "Pharmacy",
    locationBg: "#F0FDF4", locationColor: "#15803D", locationBorder: "#BBF7D0",
    department: "Pharmacy", status: "Active",
  },
  {
    id: "IN", initials: "IN", avatarBg: "#7C3AED",
    name: "ICU Nurse", username: "@icu.nurse", email: "icu@hospital.org",
    role: "Nurse",       roleBg: "#FAF5FF", roleColor: "#7E22CE",
    locationCode: "ICU-01", locationName: "ICU",
    locationBg: "#FFF7ED", locationColor: "#C2410C", locationBorder: "#FED7AA",
    department: "ICU", status: "Active",
  },
  {
    id: "TW", initials: "TW", avatarBg: "#DC2626",
    name: "Tom Williams", username: "@t.williams", email: "t.williams@hospital.org",
    role: "Storekeeper", roleBg: "#FFF7ED", roleColor: "#C2410C",
    locationCode: "LAB-01", locationName: "Laboratory",
    locationBg: "#F0F9FF", locationColor: "#0369A1", locationBorder: "#BAE6FD",
    department: "Laboratory", status: "Blocked",
  },
];

const ROLE_PERMISSION_MATRIX = [
  { role: "Manager",     color: "#1D4ED8", bg: "#EFF6FF", border: "#BFDBFE",
    view:1, add:1, edit:1, del:0, issue:1, receive:1, createPO:1, approvePO:1, transfer:1, approveTR:1, reports:1, export:1 },
  { role: "Pharmacist",  color: "#15803D", bg: "#F0FDF4", border: "#BBF7D0",
    view:1, add:1, edit:1, del:0, issue:1, receive:1, createPO:0, approvePO:0, transfer:1, approveTR:0, reports:1, export:0 },
  { role: "Storekeeper", color: "#C2410C", bg: "#FFF7ED", border: "#FED7AA",
    view:1, add:1, edit:1, del:0, issue:1, receive:1, createPO:0, approvePO:0, transfer:1, approveTR:0, reports:1, export:0 },
  { role: "Nurse",       color: "#7E22CE", bg: "#FAF5FF", border: "#E9D5FF",
    view:1, add:0, edit:0, del:0, issue:0, receive:0, createPO:0, approvePO:0, transfer:0, approveTR:0, reports:0, export:0 },
  { role: "Viewer",      color: "#6B7280", bg: "#F9FAFB", border: "#E5E7EB",
    view:1, add:0, edit:0, del:0, issue:0, receive:0, createPO:0, approvePO:0, transfer:0, approveTR:0, reports:1, export:0 },
];
...
const TH = ({ children, width, align = "left" }) => (
  <TableCell sx={{
    ...
    whiteSpace: "nowrap", bgcolor: "#F9FAFB", textAlign: align,
  }}>
    {children}
  </TableCell>
);
...
export default function UsersRoles() {
  ...
  const handleSaveUser = (form) => {
    ...
    const locCode  = form.location?.split(" ")[0]?.slice(0,3).toUpperCase() || "CS";
    ...
    const newUser  = {
      ...
      email:          form.email || `${(form.username||"user").toLowerCase()}@hospital.org`,
      role:           form.role,
      roleBg:         rc.bg,
      roleColor:      rc.color,
    ...
    <Box sx={{ bgcolor: C.bg, minHeight: "100vh" }}>
      <Box sx={{ p: "24px 28px" }}>
        ...
        <Box sx={{ display:"flex", alignItems:"center", justifyContent:"space-between", mb: 0.5 }}>
          <Box>
            ...
            <Typography sx={{ fontSize:13, color:C.textSecondary, mt:0.3 }}>
              {users.length} users · {activeCount} active · {blockedCount} blocked
            </Typography>
          </Box>
        ...
        <Box sx={{ display:"flex", alignItems:"center", gap:1.5, mb:2, mt:2, flexWrap:"wrap" }}>
          ...
          <FormControl size="small" sx={{ minWidth:140 }}>
            <Select value={roleFilter} onChange={e => setRoleFilter(e.target.value)}
              sx={{ fontSize:13, borderRadius:"8px", bgcolor:"#fff", "& .MuiOutlinedInput-notchedOutline":{ borderColor:C.border } }}>
          ...
          <TextField size="small" placeholder="Search..." value={search}
            ...
            InputProps={{ startAdornment: <InputAdornment position="start"><Search sx={{ fontSize:16, color:C.textSecondary }} /></InputAdornment> }}
            sx={{ minWidth:220, "& .MuiOutlinedInput-root":{ fontSize:13, borderRadius:"8px", bgcolor:"#fff", "& fieldset":{ borderColor:C.border } } }} />

          <Box sx={{ flex:1 }} />

          {/* Status tabs */}
          {["All","Active","Blocked"].map(tab => (
            <Button key={tab} size="small" onClick={() => setStatusTab(tab)}
              startIcon={tab === "Active" ? <CheckCircle sx={{ fontSize:13 }} /> : tab === "Blocked" ? <Block sx={{ fontSize:13 }} /> : null}
              sx={{
                fontSize:12, fontWeight:600, textTransform:"none", borderRadius:"8px",
                px:1.8, height:32,
                bgcolor: statusTab === tab
                  ? (tab === "Blocked" ? "#FEF2F2" : tab === "Active" ? "#F0FDF4" : C.primary)
                  : "#fff",
                color: statusTab === tab
                  ? (tab === "Blocked" ? "#DC2626" : tab === "Active" ? "#16A34A" : "#fff")
                  : C.textSecondary,
                border: `1px solid ${statusTab === tab
                  ? (tab === "Blocked" ? "#FECACA" : tab === "Active" ? "#BBF7D0" : C.primary)
                  : C.border}`,
                "&:hover":{ opacity:0.85 },
              }}>
              {tab}
        ...
        <TableContainer component={Paper} elevation={0}
          sx={{ border:`1px solid ${C.border}`, borderRadius:"10px", mb:3, overflow:"auto",
                ...
                <TableRow key={u.id}
                  ...
                  <TableCell sx={{ px:1.5, py:1.2 }}>
                    <Box sx={{ display:"flex", alignItems:"center", gap:1.5 }}>
                      <Avatar sx={{ width:34, height:34, bgcolor:u.avatarBg, fontSize:12, fontWeight:700 }}>
                        {u.initials}
                      </Avatar>
                      <Box>
                  ...
                  <TableCell sx={{ px:1.5, py:1.2 }}>
                    <Chip label={u.role} size="small" sx={{
                      bgcolor:u.roleBg, color:u.roleColor,
                  ...
                  <TableCell sx={{ px:1.5, py:1.2 }}>
                    <Box sx={{ display:"flex", alignItems:"center", gap:0.8 }}>
                      <Chip label={u.locationCode} size="small" sx={{
                        bgcolor:u.locationBg, color:u.locationColor, border:`1px solid ${u.locationBorder}`,
                  ...
                  {/* Status */}
                  <TableCell sx={{ px:1.5, py:1.2 }}>
                    <Chip label={u.status === "Active" ? "● Active" : "⊘ Blocked"} size="small" sx={{
                  ...
                  <TableCell sx={{ px:1.5, py:1.2 }}>
                    <Stack direction="row" spacing={0.5} justifyContent="center">
                      <Tooltip title="Edit">
                        <IconButton size="small" onClick={() => { setEditUser(u); setEditOpen(true); }} sx={{
                      ...
                      <Tooltip title={u.status === "Blocked" ? "Unblock" : "Block"}>
                        <IconButton size="small" onClick={() => toggleBlock(u.id)} sx={{
                          bgcolor: u.status === "Blocked" ? "#F0FDF4" : "#FEF2F2",
                          color:   u.status === "Blocked" ? "#16A34A" : "#DC2626",
                          "&:hover":{ bgcolor: u.status === "Blocked" ? "#DCFCE7" : "#FEE2E2" },
        ...
        <Box sx={{ mb:1.5 }}>
          <Typography sx={{ fontWeight:800, fontSize:16, color:C.textPrimary, mb:0.4 }}>
            Role Permission Matrix
          </Typography>
        </Box>

        <TableContainer component={Paper} elevation={0}
          sx={{ border:`1px solid ${C.border}`, borderRadius:"10px", overflow:"auto",
            "&::-webkit-scrollbar":{ height:4 },
            "&::-webkit-scrollbar-thumb":{ background:"#d1d5db", borderRadius:4 } }}>
          <Table size="small" sx={{ minWidth:1000 }}>
            <TableHead>
              <TableRow sx={{ bgcolor:"#F9FAFB" }}>
                <TableCell sx={{ fontWeight:700, fontSize:11, color:C.textSecondary, textTransform:"uppercase",
                  letterSpacing:0.5, py:1.4, px:1.5, borderBottom:`1px solid ${C.border}`, minWidth:130 }}>
                  ROLE
                </TableCell>
                {PERM_COLS.map(c => (
                  <TableCell key={c.key} align="center" sx={{
                    fontWeight:700, fontSize:10, color:C.textSecondary, textTransform:"uppercase",
                    letterSpacing:0.4, py:1.4, px:0.5, borderBottom:`1px solid ${C.border}`,
            ...
            <TableBody>
              {ROLE_PERMISSION_MATRIX.map((r, idx) => (
                <TableRow key={r.role}
                  sx={{ bgcolor: idx%2===0 ? "#fff":"#FAFAFA", "&:hover":{ bgcolor:"#F9FAFB" } }}>
                  <TableCell sx={{ px:1.5, py:1.1 }}>
                    <Chip label={r.role} size="small" sx={{
                      bgcolor:r.bg, color:r.color, border:`1px solid ${r.border}`,
                      fontWeight:700, fontSize:11, height:22,
                    }} />
                  </TableCell>
                  {PERM_COLS.map(c => (
