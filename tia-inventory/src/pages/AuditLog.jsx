...
const AuditLog = () => {
  ...
  const filteredLogs = useMemo(() => {
    return auditData.filter(log => {
      ...
      const matchesLocation = locationFilter ? log.action === locationFilter : true;
  ...
  return (
    <Box sx={{ p: { xs: 2, sm: 3 }, maxWidth: '1400px', mx: 'auto',marginTop: -3 }}>
      ...
      >
        ...
        <MenuItem value="PO" sx={{ fontSize: '0.75rem' }}>PO</MenuItem>
        <MenuItem value="GRN" sx={{ fontSize: '0.75rem' }}>GRN</MenuItem>
        <MenuItem value="StockIssue" sx={{ fontSize: '0.75rem' }}>StockIssue</MenuItem>
        </Select>
        ...
        <Select
          ...
          sx={{ minWidth: 140, }}
        >
          <MenuItem value="" sx={{ fontSize: '0.75rem' }}>All Locations</MenuItem>
          <MenuItem value="APPROVED" sx={{ fontSize: '0.75rem' }}>APPROVED</MenuItem>
          <MenuItem value="GRN_COMPLETE" sx={{ fontSize: '0.75rem' }}>GRN_COMPLETE</MenuItem>
          <MenuItem value="ISSUE" sx={{ fontSize: '0.75rem' }}>ISSUE</MenuItem>
        </Select>
          ...
          <Table sx={{ minWidth: 650, borderCollapse: 'collapse' }}>
            ...
            <TableBody>
              {filteredLogs.map((log) => (
                ...
                >
                  <TableCell sx={{ fontFamily: 'monospace', fontSize: '11px', whiteSpace: 'nowrap', color: '#6b7280' }}>
                    {log.timestamp}
                  </TableCell>
                  <TableCell>
                    <Box>
                      <Typography sx={{ fontSize: '12px', fontWeight: 600 }}>{log.user.name}</Typography>
                      <Typography sx={{ fontSize: '10px', color: '#9ca3af' }}>{log.user.id}</Typography>
                    </Box>
                  </TableCell>
                  <TableCell>
                    <Box
                  ...
                  <TableCell sx={{ fontSize: '12px' }}>{log.module}</TableCell>
                  <TableCell>
                    <Typography sx={{ fontSize: '12px', fontWeight: 'bold', color: '#0f766e' }}>
                      {log.record}
                    </Typography>
                  </TableCell>
                  <TableCell sx={{ fontSize: '12px', color: '#6b7280' }}>{log.detail}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
