...
const autoSt = { ...inputSt, background: "#f1f5f9", color: "#374151", cursor: "not-allowed" };

const SecLabel = ({ text }) => (
  <div style={{ fontSize: 11, fontWeight: 700, color: "#0ea5e9", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 14, marginTop: 4 }}>{text}</div>
);
const FLabel = ({ text }) => (
...
const Divider = () => <div style={{ borderTop: "1px solid #e2e8f0", margin: "18px 0" }} />;
...
function ReplaceModal({ prefill, onClose, onSubmit }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 560, boxShadow: "0 24px 64px rgba(0,0,0,0.20)", maxHeight: "90vh", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ padding: "20px 24px 16px", borderBottom: "1px solid #e2e8f0", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <button onClick={onClose} style={{ border: "1.5px solid #e2e8f0", borderRadius: 8, width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center", background: "#fff", cursor: "pointer", color: "#64748b", fontSize: 18, flexShrink: 0 }}>×</button>
        </div>
        ...
        <div className="modal-body" style={{ padding: "20px 24px", overflowY: "auto", flex: 1 }}>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <input value={form.item} readOnly={!!prefill} onChange={e => set("item", e.target.value)}
              style={prefill ? autoSt : inputSt} placeholder="Select item..." />
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
            <div><FLabel text="Current QTY (Auto)" /><input readOnly value={form.currentQty} style={autoSt} /></div>
            <div><FLabel text="Location (Auto)" /><input readOnly value={form.location} style={autoSt} /></div>
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
            <div>
              <FLabel text="Replacement Reason *" />
              <select value={form.reason} onChange={e => set("reason", e.target.value)} style={selectSt}>
                {["Expired", "Low Stock", "Recalled", "Damaged"].map(r => <option key={r}>{r}</option>)}
              </select>
            </div>
            <div>
              <FLabel text="Urgency *" />
              <select value={form.urgency} onChange={e => set("urgency", e.target.value)} style={selectSt}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            <div><FLabel text="QTY Disposed / Removed" /><input type="number" value={form.disposed} onChange={e => set("disposed", e.target.value)} placeholder="0" style={inputSt} /></div>
            <div><FLabel text="QTY to Replace *" /><input type="number" value={form.replaceQty} onChange={e => set("replaceQty", e.target.value)} placeholder="0" style={inputSt} /></div>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: form.useSubstitute ? 14 : 0 }}>
            <input type="checkbox" id="useSub" checked={form.useSubstitute} onChange={e => set("useSubstitute", e.target.checked)}
              style={{ width: 16, height: 16, cursor: "pointer", accentColor: "#2563eb" }} />
            <label htmlFor="useSub" style={{ fontSize: 13, color: "#374151", cursor: "pointer", userSelect: "none" }}>Use a substitute / alternative item</label>
          </div>

          {form.useSubstitute && (
            <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 14 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                <div><FLabel text="Substitute Name" /><input value={form.substitute} onChange={e => set("substitute", e.target.value)} placeholder="e.g. Generic Amox 500mg" style={inputSt} /></div>
                <div><FLabel text="Substitute NDC" /><input value={form.substituteNDC} onChange={e => set("substituteNDC", e.target.value)} placeholder="0378-0255-01" style={inputSt} /></div>
              </div>
              <div>
                <FLabel text="Therapeutic Equivalence" />
                <select value={form.therapeuticEq} onChange={e => set("therapeuticEq", e.target.value)} style={selectSt}>
                  {THERAPEUTIC_EQ.map(o => <option key={o}>{o}</option>)}
                </select>
              </div>
            </div>
          )}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
            <div>
              ...
              <select value={form.supplier} onChange={e => set("supplier", e.target.value)} style={selectSt}>
                {SUPPLIERS.map(s => <option key={s}>{s}</option>)}
              </select>
            </div>
            <div><FLabel text="Est. Unit Cost" /><input type="number" value={form.unitCost} onChange={e => set("unitCost", e.target.value)} placeholder="0.00" style={inputSt} /></div>
          </div>
        ...
        <div style={{ padding: "16px 24px", borderTop: "1px solid #e2e8f0", display: "flex", gap: 10, justifyContent: "flex-end", alignItems: "center" }}>
          <button onClick={onClose} style={{ padding: "9px 20px", border: "1.5px solid #e2e8f0", borderRadius: 8, background: "#fff", cursor: "pointer", fontSize: 13, fontWeight: 600, color: "#374151" }}>Cancel</button>
          <button onClick={() => handleSave(false)} style={{ padding: "9px 18px", border: "1.5px solid #e2e8f0", borderRadius: 8, background: "#fff", cursor: "pointer", fontSize: 13, fontWeight: 600, color: "#374151", display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <button onClick={() => handleSave(true)} style={{ padding: "9px 18px", border: "none", borderRadius: 8, background: "rgb(37, 99, 235)", cursor: "pointer", fontSize: 13,  fontWeight: 700, color: "#fff", display: "flex", alignItems: "center", gap: 6, boxShadow: "0 2px 8px rgba(14,116,144,0.3)" }}>
...
function DisposeModal({ item, onClose, onConfirm }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 420, boxShadow: "0 24px 64px rgba(0,0,0,0.20)", padding: 28 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
          ...
          <div>
            <div style={{ fontSize: 15, fontWeight: 800, color: "#0f172a" }}>Confirm Disposal</div>
            <div style={{ fontSize: 12, color: "#94a3b8" }}>This action cannot be undone</div>
          </div>
        </div>
        <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 8, padding: "12px 14px", marginBottom: 20, fontSize: 13, color: "#374151" }}>
          Mark <strong>{item.item}</strong> (Lot: {item.lot}, Qty: {item.qty}) at <strong>{item.location}</strong> as disposed?
        </div>
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
          <button onClick={onClose} style={{ padding: "9px 20px", border: "1.5px solid #e2e8f0", borderRadius: 8, background: "#fff", cursor: "pointer", fontSize: 13, fontWeight: 600, color: "#374151",}}>Cancel</button>
          <button onClick={() => onConfirm(item.id)} style={{ padding: "9px 20px", border: "none", borderRadius: 8, background: "#ef4444", cursor: "pointer", fontSize: 13, fontWeight: 700, color: "#fff" }}>Dispose</button>
        </div>
      </div>
    </div>
  );
...
export default function ExpiryTracking() {
  ...
  const handleExport = () => {
    ...
    const rows = displayed.map(r => [r.item, r.ndc, r.location, r.lot, r.qty, formatDate(r.expiry), r.days < 0 ? `${Math.abs(r.days)}d ago` : `${r.days}d`, r.status]);
    const csv = [headers, ...rows].map(row => row.join(",")).join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url; a.download = "expiry_tracking.csv"; a.click();
  ...
  return (
    <div style={{  background: "#f8fafc", minHeight: "100vh", padding: "22px 20px" }}>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .filter-btn.active { background: #f0f9ff; border-color: #0ea5e9; color: #0ea5e9; font-weight: 700; }
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 26, fontWeight: 800, color: "#0f172a" }}>Expiry Tracking</h1>
          <div style={{ fontSize: 13, color: "#94a3b8", marginTop: 4 }}>
            <span style={{ color: "#ef4444", fontWeight: 600 }}>{expired.length} expired</span>
            <span style={{ margin: "0 6px" }}>·</span>
            <span style={{ color: "#d97706", fontWeight: 600 }}>{expiringSoon.length} expiring within 60 days</span>
          </div>
        </div>
        <button onClick={handleExport} style={{ display: "flex", alignItems: "center", gap: 6, padding: "9px 18px", border: "1.5px solid #e2e8f0", borderRadius: 9, background: "#fff", cursor: "pointer", fontSize: 13, fontWeight: 600, color: "#374151", boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
      ...
      <div style={{ display: "flex", gap: 16, marginBottom: 28 }}>
        {[
          { key: "EXPIRED",       count: expired.length,      sub: "Must be disposed immediately", accent: "#ef4444" },
          { key: "EXPIRING ≤ 60 DAYS", count: expiringSoon.length, sub: "Plan replacement now",     accent: "#d97706" },
          { key: "OK",            count: ok.length,           sub: "No immediate concern",           accent: "#16a34a" },
        ].map(({ key, count, sub, accent }) => (
          <div key={key} style={{ flex: 1, background: "#fff", borderRadius: 14,  overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
            <div style={{ height: 4, background: accent }} />
            <div style={{ padding: "18px 22px" }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: "#94a3b8", letterSpacing: "0.07em", textTransform: "uppercase", marginBottom: 8 }}>{key}</div>
              <div style={{ fontSize: 36, fontWeight: 800, color: "#0f172a", lineHeight: 1 }}>{count}</div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
        {[
          ...
          <button key={label} className={`filter-btn ${filter === label ? "active" : ""}`} onClick={() => setFilter(label)}>
      ...
      <div style={{ background: "#fff", borderRadius: 14, border: "1.5px solid #e2e8f0", overflow: "hidden", boxShadow: "0 1px 6px rgba(0,0,0,0.05)" }}>
            ...
            <thead>
              <tr style={{ background: "#f8fafc", borderBottom: "1.5px solid #e2e8f0" }}>
                {["Item", "NDC", "Location", "Lot #", "QTY", "Expiry Date", "Days Left", "Status", "Action"].map(h => (
                  <th key={h} style={{ padding: "11px 10px", textAlign: "left", fontSize: 10.5, fontWeight: 700, color: "#94a3b8", letterSpacing: "0.06em", textTransform: "uppercase", whiteSpace: "nowrap", overflow: "hidden" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {displayed.length === 0 ? (
                <tr><td colSpan={9} style={{ padding: 48, textAlign: "center", color: "#94a3b8", fontSize: 14 }}>No records found.</td></tr>
              ) : displayed.map((row, i) => {
                ...
                return (
                  <tr key={row.id} className="row" style={{ borderBottom: i < displayed.length - 1 ? "1px solid #f1f5f9" : "none" }}>
                    <td style={{ padding: "11px 10px", overflow: "hidden" }}>
                      <div style={{ fontSize: 12.5, fontWeight: 600, color: "#0f172a", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{row.item}</div>
                    </td>
                    <td style={{ padding: "11px 10px", fontSize: 11.5, color: "#64748b", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{row.ndc}</td>
                    <td style={{ padding: "11px 10px" }}>
                      <span style={{ background: lc.bg, color: lc.color, padding: "2px 8px", borderRadius: 5, fontSize: 11, fontWeight: 700, whiteSpace: "nowrap" }}>{row.location}</span>
                    </td>
                    <td style={{ padding: "11px 10px", fontSize: 12, color: "#374151", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{row.lot}</td>
                    <td style={{ padding: "11px 10px", fontSize: 12.5, fontWeight: 600, color: "#0f172a", textAlign: "center" }}>{row.qty}</td>
                    <td style={{ padding: "11px 10px", fontSize: 12, fontWeight: 600, color: isExpired ? "#ef4444" : row.days <= 60 ? "#d97706" : "#374151", whiteSpace: "nowrap", textDecoration: isExpired ? "line-through" : "none", overflow: "hidden" }}>
                      {formatDate(row.expiry)}
                    </td>
                    <td style={{ padding: "11px 10px" }}>
                      <span style={{ background: dl.bg, color: dl.color, padding: "2px 8px", borderRadius: 5, fontSize: 11, fontWeight: 700, whiteSpace: "nowrap" }}>{dl.label}</span>
                    </td>
                    <td style={{ padding: "11px 10px" }}>
                      <span style={{ background: sc.bg, color: sc.color, border: `1.5px solid ${sc.border}`, padding: "2px 8px", borderRadius: 5, fontSize: 11, fontWeight: 700, whiteSpace: "nowrap" }}>{sc.label}</span>
                    </td>
                    <td style={{ padding: "11px 10px" }}>
                      <div style={{ display: "flex", gap: 5, alignItems: "center" }}>
                        {isExpired && (
                          <button className="action-btn" onClick={() => setDisposeTarget(row)}
                            style={{ borderColor: "#fecaca", color: "#ef4444", background: "#fff", padding: "4px 10px", fontSize: 11 }}>
                            Dispose
                          </button>
                        )}
                        <button className="action-btn" onClick={() => setReplaceTarget(row)}
                          style={{ borderColor: "#bfdbfe", color: "#1d4ed8", background: "#eff6ff", padding: "4px 10px", fontSize: 11 }}>
        ...
        <div style={{ padding: "11px 16px", borderTop: "1px solid #f1f5f9", fontSize: 12, color: "#94a3b8", display: "flex", justifyContent: "space-between" }}>
          <span>Showing {displayed.length} of {enriched.length} records</span>
