...
const calcDaysLeft = (dateStr) => Math.round((new Date(dateStr) - today) / (1000 * 60 * 60 * 24));
...
const getStatus = (days) => days < 0 ? "Expired" : days <= 60 ? "Expiring Soon" : "OK";
...
const SecLabel = ({ text }) => (
  <Typography sx={{ fontSize:11, fontWeight:700, color:"#2563eb", letterSpacing:"0.08em", textTransform:"uppercase", mb:1.5, mt:0.5 }}>{text}</Typography>
);
const FLabel = ({ text }) => (
  <Typography component="label" sx={{ display:"block", fontSize:11, fontWeight:700, color:"#64748b", letterSpacing:"0.06em", textTransform:"uppercase", mb:0.75 }}>{text}</Typography>
);
...
const autoSx = { ...inputSx, "& .MuiInputBase-root":{ fontSize:13, borderRadius:"8px", bgcolor:"#f1f5f9", color:"#374151" } };

const locColor = (loc) => {
  const map = { "CS-01":{ bg:"#ede9fe", color:"#6d28d9" }, "PH-01":{ bg:"#dcfce7", color:"#166534" }, "OR-01":{ bg:"#ffedd5", color:"#c2410c" }, "WA-01":{ bg:"#dbeafe", color:"#1e40af" }, "WA-02":{ bg:"#fce7f3", color:"#9d174d" } };
...
function ReplaceModal({ prefill, onClose, onSubmit }) {
  ...
  const handleSave = (raisePO) => { if (!form.item || !form.replaceQty) return; onSubmit({ ...form, raisePO }); };
      ...
      <Box sx={{ position:"fixed", inset:0, display:"flex", alignItems:"center", justifyContent:"center", p:2 }}>
        <Box sx={{ bgcolor:"#fff", borderRadius:"16px", width:"100%", maxWidth:420, boxShadow:"0 24px 64px rgba(0,0,0,0.20)", p:3.5 }}>
          ...
          <Box sx={{ display:"flex", gap:1.25, justifyContent:"flex-end" }}>
            <Button onClick={onClose} variant="outlined" sx={{ textTransform:"none", fontSize:13, fontWeight:600, color:"#374151", borderColor:"#e2e8f0", borderRadius:"8px", "&:hover":{ borderColor:"#cbd5e1", bgcolor:"#f8fafc" } }}>Cancel</Button>
            <Button onClick={() => onConfirm(item.id)} variant="contained" sx={{ textTransform:"none", fontSize:13, fontWeight:700, bgcolor:"#ef4444", borderRadius:"8px", "&:hover":{ bgcolor:"#dc2626" }, boxShadow:"none" }}>Dispose</Button>
          </Box>
        </Box>
      </Box>
    </Modal>
...
export default function ExpiryTracking() {
  ...
  const enriched     = items.map((it) => { const days = calcDaysLeft(it.expiry); return { ...it, days, status:getStatus(days) }; });
  ...
  const displayed    = filter === "Expired" ? expired : filter === "Expiring Soon" ? expiringSoon : enriched;

  const handleDispose = (id) => { setItems((prev) => prev.filter((i) => i.id !== id)); setDisposeTarget(null); };
  ...
  const handleExport = () => {
    ...
    const a = document.createElement("a"); a.href = URL.createObjectURL(new Blob([csv], { type:"text/csv" })); a.download = "expiry_tracking.csv"; a.click();
  };

  const daysDisplay  = (days) => days < 0 ? { label:`${Math.abs(days)}d ago`, color:"#ef4444", bg:"#fee2e2" } : days <= 60 ? { label:`${days}d`, color:"#d97706", bg:"#fef3c7" } : { label:`${days}d`, color:"#16a34a", bg:"#dcfce7" };
  const statusChip   = (status) => status === "Expired" ? { label:"Expired", bg:"#fee2e2", color:"#ef4444", border:"#fecaca" } : status === "Expiring Soon" ? { label:"Expiring Soon", bg:"#fef9c3", color:"#d97706", border:"#fde68a" } : { label:"OK", bg:"#dcfce7", color:"#16a34a", border:"#bbf7d0" };

  const filters = [{ label:"All", dot:null }, { label:"Expired", dot:"#ef4444" }, { label:"Expiring Soon", dot:"#d97706" }];

  const statCards = [
    { label:"Expired",          value:expired.length,      sub:"Must dispose immediately", iconBg:"#ef4444", icon:<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg> },
    { label:"Expiring ≤ 60 Days",value:expiringSoon.length, sub:"Plan replacement now",    iconBg:"#f59e0b", icon:<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg> },
    { label:"OK",               value:ok.length,           sub:"No immediate concern",     iconBg:"#10b981", icon:<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg> },
  ];
  ...
  return (
    <Box>
      ...
      <Box sx={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", mb:3 }}>
        ...
        <Button onClick={handleExport} startIcon={<DownloadOutlinedIcon sx={{ fontSize:"14px !important" }} />} variant="outlined" sx={btnOutlined}>
          Export
        </Button>
      </Box>
      ...
      <Paper elevation={0} sx={{ borderRadius:"14px", border:"1.5px solid #e2e8f0", overflow:"hidden", boxShadow:"0 1px 6px rgba(0,0,0,0.05)" }}>
        <TableContainer>
          <Table sx={{ tableLayout:"fixed", minWidth:900 }}>
            ...
            <TableBody>
              {displayed.length === 0 ? (
                <TableRow><TableCell colSpan={9} sx={{ py:6, textAlign:"center", color:"#94a3b8", fontSize:14, border:"none" }}>No records found.</TableCell></TableRow>
              ) : displayed.map((row, i) => {
                ...
                return (
                  <TableRow key={row.id} sx={{ borderBottom:i < displayed.length-1?"1px solid #f1f5f9":"none", "&:hover td":{ bgcolor:"#fafafa" } }}>
                    <TableCell sx={{ py:"11px", px:"10px", border:"none" }}><Typography sx={{ fontSize:13, fontWeight:500, color:"#2E2E2E", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{row.item}</Typography></TableCell>
                    ...
                    <TableCell sx={{ py:"11px", px:"10px", border:"none" }}><Chip label={row.location} size="small" sx={{ bgcolor:lc.bg, color:lc.color, fontWeight:700, fontSize:11, height:22, borderRadius:"5px" }} /></TableCell>
                    <TableCell sx={{ py:"11px", px:"10px", fontSize:13, color:"#374151", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", border:"none" }}>{row.lot}</TableCell>
                    <TableCell sx={{ py:"11px", px:"10px", fontSize:12.5, fontWeight:500, color:"#0f172a", textAlign:"center", border:"none" }}>{row.qty}</TableCell>
                    <TableCell sx={{ py:"11px", px:"10px", border:"none" }}><Typography sx={{ fontSize:12, fontWeight:600, color:isExpired?"#ef4444":row.days<=60?"#d97706":"#374151", whiteSpace:"nowrap", textDecoration:isExpired?"line-through":"none" }}>{formatDate(row.expiry)}</Typography></TableCell>
                    <TableCell sx={{ py:"11px", px:"10px", border:"none" }}><Chip label={dl.label} size="small" sx={{ bgcolor:dl.bg, color:dl.color, fontWeight:700, fontSize:11, height:22, borderRadius:"5px" }} /></TableCell>
                    <TableCell sx={{ py:"11px", px:"10px", border:"none" }}><Chip label={sc.label} size="small" variant="outlined" sx={{ bgcolor:sc.bg, color:sc.color, borderColor:sc.border, fontWeight:700, fontSize:11, height:22, borderRadius:"5px" }} /></TableCell>
                    <TableCell sx={{ py:"11px", px:"10px", border:"none" }}>
                      <Box sx={{ display:"flex", gap:0.75, alignItems:"center" }}>
                        {isExpired && (
                          <Button onClick={() => setDisposeTarget(row)} size="small" variant="outlined"
                        ...
                        {/* ── Replace — Replacement style (outlined) ── */}
                        <Button onClick={() => setReplaceTarget(row)} size="small" startIcon={<SyncAltOutlinedIcon sx={{ fontSize:"11px !important" }} />}
                          sx={{ textTransform:"none", fontSize:11, fontWeight:700, color:"#015DFF", bgcolor:"#EFF4FF", border:"1.5px solid #015DFF", borderRadius:"7px", px:1.25, py:0.5, minWidth:0, "& .MuiButton-startIcon":{ mr:0.5 }, "&:hover":{ bgcolor:"#dbeafe", borderColor:"#015DFF" } }}>
                          Replace
                        </Button>
                      </Box>
                    </TableCell>
